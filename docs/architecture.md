...
一个汇聚主流 AI 大模型评测数据的排行榜网站，支持多维度排行对比、模型详情查看、雷达图对比等功能。数据通过定时抓取公开评测平台获得（每 3 天自动更新）。
...
| 定时任务 | node-cron | 4.x | 每 3 天自动抓取数据 |
...
│                 Next.js API Routes                    │
│  ┌────────────┐ ┌──────────┐ ┌───────────────────┐   │
│  │/api/rankings│ │/api/models│ │/api/scraper/trigger│  │
│  └─────┬──────┘ └────┬─────┘ └─────────┬─────────┘  │
└────────┼─────────────┼─────────────────┼─────────────┘
         │             │                 │
┌────────▼─────────────▼─────────────────▼─────────────┐
...
│   └── migrations/            # 数据库迁移文件
├── src/
│   ├── instrumentation.ts     # 应用启动钩子（启动定时抓取）
...
触发方式:
  ...
  2. GET  /api/scraper/trigger  (查看抓取历史)
  3. node-cron 定时任务 (每 3 天凌晨 2:00，通过 instrumentation.ts 启动)
...
- 接入更多真实数据源 API（LMSYS Arena API、HuggingFace API）
- 增加用户自定义权重功能
- 增加模型价格/性价比维度
- 支持多语言界面（i18n）
- 增加 API 鉴权（保护 scraper trigger 接口）
